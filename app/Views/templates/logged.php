    ...
    <script>
        const message = (status = "200", message) => {
            let html = `<div class="d-flex justify-content-center">
                            <div class="bg-opacity-25 ${(status=="200"?"bg-success border border-success":"bg-danger border border-danger")} px-5 rounded" style="font-size: x-small;">${message}</div>
                        </div>`;
        ...
        function date(type) {
            const today = new Date(); // Ambil tanggal saat ini

            switch (type.toLowerCase()) {
                case 'y':
                    return today.getFullYear(); // Mengembalikan tahun
                case 'm':
                    return today.getMonth() + 1; // Mengembalikan bulan (ditambah 1 karena Januari = 0)
                case 'd':
                    return today.getDate(); // Mengembalikan tanggal
                default:
                    return 'Parameter tidak valid. Gunakan "year", "month", atau "date".';
            }
        }
    </script>
...
<body class="bg_main text_main" style="padding-bottom: 80px;">
    ...
    <audio id="audio_notif_Kantin" src="<?= base_url('files/audio/notif_kantin.mp3'); ?>" loop></audio>
    <audio id="audio_notif_Disiplin" src="<?= base_url('files/audio/notif_disiplin.mp3'); ?>" loop></audio>
    <audio id="audio_notif_Sos" src="<?= base_url('files/audio/notif_sos.mp3'); ?>" loop></audio>
    ...
    <div class="modal fade" id="notif" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered px-5">
            <div class="modal-content">
                <div class="modal-body border_main bg_main rounded modal_body_notif" style="margin-top: -300px;">
    ...
    <script>
        ...
        <?php if (menu()['controller'] == "home" && url(4) == ""): ?>
            setInterval(() => {
                notif();
            }, 5000);
        <?php endif; ?>

        const modal_notif = (data) => {
            ...
            let html = `<div class="d-flex justify-content-between mb-4">
                        <div style="font-size:small;">Notif</div>`;
            if (data.button) {
                html += `<a href="" class="text_main" data-bs-dismiss="modal" type="button" aria-label="Close" style="font-size: small;"><i class="fa-solid fa-xmark"></i></a>`;
            }
            html += `</div>

                    <div class="p-2 rounded text-center fs-6 mb-1">
                        <div class="position-relative" style="display: inline-block;">
                            <div class="spinner-border p-1" style="width:50px;height:50px" role="status"></div>
                            <div class="position-absolute top-50 start-50 translate-middle durasi" style="animation: none;font-size:20px;margin-top:-3px;"><i class="fa-solid fa-bell"></i></div>
                        </div>
                    </div>
                    <h6 class="text-center mb-5">${data.judul}</h6>
                    <div class="d-grid">
                        <a href="${data.link}" data-id="${data.data.id}" data-judul="${data.judul}" class="text-center bg_secondary btn btn-sm py-2 px-3 rounded text_main btn_notif">${data.message}</a>
                    </div>`;

            $(".modal_body_notif").html(html);

            if (data.judul == "Listrik & Kebersihan") {
                data.judul = "Disiplin";
            }
            $('#audio_notif_' + data.judul)[0].play();
            modal.show();
        }
        ...
        const stop_audio = () => {
            let audio = $('#audio_notif_Kantin')[0];
            audio.pause();
            audio.currentTime = 0;
        }
        ...
        class Modal {
            ...
            akhiri(cls, message = "Yakin akhiri permainan?") {
                let html = `<div class="d-flex justify-content-center mt-5">
                            <div class="rounded text-center text-warning border bg-warning bg-opacity-10 border-warning px-4 py-2">
                                <div class="d-flex justify-content-between gap-3">
                                    <div class="fs-6 pt-1"><i class="fa-solid fa-circle-check"></i> ${message}</div>
                ...
                html += ` class="btn btn-sm btn-success btn_akhiri"><i class="fa-solid fa-circle-check"></i></button>
                                        <button class="btn btn-sm btn-secondary" data-bs-dismiss="modal"><i class="fa-solid fa-ban"></i></button>
                                    </div>
                                </div>
                            </div>
            ...
            durasi(cls, durasi) {
                let html = `<div class="d-flex justify-content-center mt-5">
                            <div class="rounded text-center text-warning border bg-warning bg-opacity-10 border-warning px-4 py-2">
                                <div class="d-flex justify-content-between gap-3">
                                    <div class="fs-6 pt-1"><i class="fa-solid fa-circle-check"></i> Yakin tambah ${durasi} Jam?</div>
                                    <div>
                                        <button `;
                let dataAttr = getDataAttr(cls);
                dataAttr.forEach(e => {
                ...
                html += ` data-durasi="${durasi}" class="btn btn-sm btn-success btn_tambah_durasi"><i class="fa-solid fa-circle-check"></i></button>
                                        <button class="btn btn-sm btn-secondary" data-bs-dismiss="modal"><i class="fa-solid fa-ban"></i></button>
                                    </div>
                                </div>
                            </div>
        ...
        function metode_tap(data, jenis, limit = 31) {
            let html = `
                    <div class="fixed-top bg_main border_main rounded text_main px-2 py-5" style="z-index: 99999;height:100%;padding-top:11px">
                        <div class="text-center">
                            <div class="position-relative" style="display: inline-block;">
                                <div class="spinner-border p-1" style="width:50px;height:50px" role="status"></div>
                                <div class="position-absolute top-50 start-50 translate-middle durasi" style="animation: none;font-size:20px;margin-top:-3px;">${limit}</div>
                            </div>
                            <div class="text-center mt-3">
                                <div class="pembeli border_bottom fs-2"></div>
                                <div class="fs-6">
                                    <div style="font-size: 10px;">DANA TERSIMPAN</div>
                                    <div class="fw-bold fulus"></div>
                                </div>
                                <div class="fs-6 mt-3 px-4">
                                    <div class="bg_secondary rounded p-2">
                                        <div style="font-size: 10px;">TOTAL TRANSAKSI</div>
                                        <div class="fw-bold total"></div>
                                    </div>
                                </div>
                                <div class="pesan mt-5">Proses...</div>
                                <div class="fs-6 saldo"></div>
                            </div>
                        </div>
                    </div>`;
            ...
            let newId = 0;
            let langkah = "";
            const checkLopp = async () => {
                if (limit <= 0) {
                    clearInterval(loopReq);
                    $('.durasi').text(0);
                    $(".pesan").html('<span class="text-danger fw-bold">Waktu Habis!.</span>');

                    post("home/delete_id_metode_tap", {
                        newId
                    }).then(res => {
                        if (res.status == "400") {
                ...
                try {
                    const res = await post("home/cek_metode_tap", {
                        newId,
                        langkah,
                        jenis,
                        data,
                    ...
                    if (res.status == "200") {
                        ...
                        if (res.data3 !== null) {
                            $(".pembeli").text(res.data3.pembeli);
                            $(".fulus").text(res.data3.fulus);
                            $(".total").text(angka(res.data3.total));
                        ...
                        if (res.data2 == "Tap") {
                            const exeLoop = async () => {
                                clearInterval(loopReq);
                                try {
                                    const respond = await post("home/metode_tap", {
                                        newId,
                                        data,
                                        lokasi
                                    });
                                    ...
                                    if (respond.status) {
                                        $(".pesan").text(respond.message);

                                        if (respond.status == "200") {
                                            let html = `<div style="font-size: 10px;">SALDO</div>
                                                        <div class="fw-bold">${angka(respond.data.saldo)}</div>`;

                                            if (respond.data.err.length > 0) {
                                                html += `<div class="my-2" style="font-size: 11px;">Gagal: ${respond.data.err.join(", ")}</div>`;
                                            }
                                    ...
                                    // $(".pesan").text("Terjadi kesalahan dalam proses.");
                                    console.error(error);
                                }
                            }
                            exeLoop();
                        ...
                        $(".pesan").text(res.message);
                        $(".fulus").text(res.data);
                        $(".total").text(res.data2);
                        if (res.message == "Saldo tidak cukup!") {
                            let x = 0;
                            setInterval(() => {
                                x++;
                                if (x == 3) {
                                    $(".pesan").text("Proses Batal!");
                                }
                                if (x == 5) {
                                    $(".div_top").html("");
                                }
            ...
            const loopReq = setInterval(checkLopp, 1000);
